...
class QuestionSearchBlock extends Component
{
    
    public $sub_title, $title, $description, $list_title, $question_list =[];
    ...
    {
        return view('livewire.pagebuilder.question-search-block');
    }
    ...
    public function mount($page_id, $block_key, $settings, $style_css, $site_view){
        ...
        if( !empty( $settings) ){
            ...
            $this->list_title           = !empty($settings['list_title']) ? $settings['list_title'] : '';
            $this->question_list        = !empty($settings['question_list']) ? $settings['question_list'] : array();

        }else{
            $getDefaultValues = SiteSetting::where('setting_type', 'question-search-block')->get();
