    ...
    <div class ="row">
        ...
        <div class="col-lg-8 col-md-12 tb-md-60">
            <div class="tb-dhb-mainheading">
                <h4> {{ __('expert_levels.text') }}</h4>
                <div class="tb-sortby">
                    <form class="tb-themeform tb-displistform">
                        <fieldset>
                            <div class="tb-themeform__wrap">
                                <div class="tb-actionselect">
                                    <a href="javascript:;" class="tb-btn btnred {{ $selectedExpertLevels ? '' : 'd-none' }}" onClick="confirmation()">{{ __('general.delete_selected') }}</a>
                                </div>
                                <div class="tb-actionselect">
                                    <div class="tb-select">
                                ...
                                <div class="form-group tb-inputicon tb-inputheight">
                                    ...
                                    <input type="text" class="form-control" wire:model.debounce.500ms="search"  autocomplete="off" placeholder="{{ __('expert_levels.search') }}">
                                </div>
                            </div>
                        </fieldset>
                    </form>
            ...
            <div class="tb-disputetable">
                @if(!empty($expertLevels) && $expertLevels->count() > 0)
                    <table class="table tb-table tb-dbholder">
                        <thead>
                            <tr>
                                ...
                                <div class="tb-checkbox">
                                    ...
                                    <label for="checkAll"><span> {{ __('expert_levels.name') }}   </span></label>
                                </div>
                                </th>
                        ...
                        <tbody>
                            @foreach($expertLevels as $single)
                                <tr>
                                    <td data-label="{{ __('general.title') }}">
                                        <div class="tb-checkboxwithimg">
                                            <div class="tb-checkbox">
                                                <input id="expertlevel_id{{ $single->id }}" wire:model.lazy="selectedExpertLevels" value="{{ $single->id }}" type="checkbox">
                                                <label for="expertlevel_id{{ $single->id }}">
                                                    <span> 
                                                        {!! $single->name !!}
                                                    </span>
                                                </label>
                        ...
                        {{ $expertLevels->links('pagination.custom') }}  
                    @else
